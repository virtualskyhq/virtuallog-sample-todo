...
export default defineConfig(({ mode }) => {
  // Read the repo-root .env directly (third arg '' = no prefix filter) so the
  // proxy target tracks the same PORT the Express server binds to. The server
  // defaults to 3000 when PORT is unset, so mirror that default here.
